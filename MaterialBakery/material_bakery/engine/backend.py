...
def ensure_image(name, size, channel, file_format='PNG', reuse=True):
    """取得（或新建）一张烘焙目标图像。
...
class CyclesBackend(BakeBackend):
    ...
    def prepare(self, task):
        ...
        image = ensure_image(task.image_name, task.render_size, task.bake_type.channel,
                             file_format=self.file_format, reuse=True)
        task.image = image
        return image
    ...
    def dispatch(self, task):
        ...
        kwargs = {
            ...
            "margin": plan_mod.effective_margin(task.size, self.settings),
