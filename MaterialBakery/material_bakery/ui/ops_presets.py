...
def _error(operator, message):
    """报一个红字，并写进会话日志（与 ui/ops.py 里同名函数一致）

    ⚠ 这个文件里原本有 **4 处** `_error(...)` 调用，但从来没有定义过它。
      走到任何一条错误分支都是 `NameError` —— 而 Blender 会吞掉 operator 里的
      异常并返回 `CANCELLED`，所以测试里那句"没烘过时存报告会被拒"一直是
      **假绿**：它分不清"礼貌地拒绝了"和"崩了"。真机上的表现则是用户只看到
      一个红字/什么都没发生，日志里一个字都没有。
      现在补上定义，并让错误真的进日志。
    """
...
def capture(settings):
    ...
    """
    maps = [{"type": item.type_key, "size_x": item.size_x, "size_y": item.size_y,
             "size": max(item.size_x, item.size_y), "enabled": item.enabled,
...
def apply_to(settings, data):
    ...
    for entry in data.get("maps", []):
        ...
        legacy = int(entry.get("size", 2048))
        item.size_x = int(entry.get("size_x", legacy) or legacy)
        item.size_y = int(entry.get("size_y", legacy) or legacy)
