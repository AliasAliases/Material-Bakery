...
def _draw_texture_groups(layout, settings):
    ...
    for group_name, entries in groups:
        ...
        sizes = sorted({item.size for _index, item in entries})
        ...
        row.label(text="/".join(naming.size_label(s) for s in sizes))
        ...
        for index, item in entries:
            ...
            sub.label(text=naming.size_label(item.size))
...
class MBAKERY_PT_Wizard(bpy.types.Panel):
    ...
    def draw_targets(self, layout, context, settings):
        ...
        for index, item in enumerate(settings.maps):
            ...
            selected = index == settings.map_index
            # ⚠ 原来这里只有一支箭头、没有文字（又一个"光秃秃的图标"）。
            #   标出"这个烘焙项的专属参数显示在下面"，用户才知道箭头是什么意思。
            row.label(text="options below" if selected else "",
                      icon='RIGHTARROW' if selected else 'BLANK1')
        ...
        for group in groups:
            ...
            for item in settings.maps[:2]:
                ...
                name = naming.render_name(settings.template, {
                    ...
                    "size": naming.size_token(item.size),
    ...
    def draw_settings(self, layout, context, settings):
        ...
        if settings.template:
            sample = naming.render_name(settings.template, {
                ...
                "size": naming.size_token(2048), "suffix": settings.suffix})
