...
def migrate(data):
    ...
    if version < 4:
        data = _migrate_v3_to_v4(data, notes)
        version = 4
...
def _migrate_v0_to_v1(data, notes):
    ...
    for entry in data.get("maps", []) or []:
        ...
        if bake_types.get(type_key) is None:
            notes.append("dropped unknown map '{}'".format(type_key))
            continue
        legacy = int(entry.get("size", 2048) or 2048)
        maps.append({
            "type": type_key,
            "size": legacy,
...
def _migrate_v3_to_v4(data, notes):
    """v3 -> v4：分辨率从"一个正方形尺寸"变成"宽 + 高"两条边

    用户 2026-09："凭什么只能烘正方形的贴图" —— 第一页现在是两个框。
    旧预设里每个 map 只有 `size`（正方形），迁移成 size_x = size_y = size，
    所以老预设读进来**行为完全不变**（还是正方形，还是原来那个分辨率）。
    """
    for entry in data.get("maps", []) or []:
        if not isinstance(entry, dict):
            continue
        legacy = int(entry.get("size", 2048) or 2048)
        entry["size_x"] = int(entry.get("size_x", legacy) or legacy)
        entry["size_y"] = int(entry.get("size_y", legacy) or legacy)
        entry["size"] = max(entry["size_x"], entry["size_y"])
    if data.get("maps"):
        notes.append("map sizes migrated to width/height pairs")
    return data
...
def describe(name):
    ...
    size = 0
    sizes = [max(int(entry.get("size_x", 0) or 0),
                 int(entry.get("size_y", 0) or 0),
