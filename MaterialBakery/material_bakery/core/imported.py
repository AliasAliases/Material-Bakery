...
def parse_size(token):
    """'2k' -> 2048，'2048' -> 2048，都不像就返回 0"""
    match = _SIZE_TOKEN.match(str(token or "").strip())
    if not match:
        return 0
...
def parse_filename(filename, tokens=None, fallback_size=0):
    """从文件名反解 (type_key, size, prefix)。认不出来返回 (None, size, prefix)
    ...
    if not parts:
        return None, fallback_size, ""

    # 1) 从右往左扒掉尺寸 token（可能没有）
    size = fallback_size
    while parts and parse_size(parts[-1]) and (len(parts) > 1):
        size = parse_size(parts.pop())
    ...
    if type_key is None:
        ...
        if glued_key is not None:
            ...
            return glued_key, size, prefix.strip()
        # 类型认不出来时**不猜**，但把可能的前缀还给调用方
        return None, size, " ".join(parts)

    prefix = " ".join(parts[:len(parts) - consumed]) if consumed else " ".join(parts)
    return type_key, size, prefix.strip()
...
class FoundTexture:
    ...
    __slots__ = ("group", "type_key", "size", "path", "source")

    def __init__(self, group, type_key, size, path, source="filename"):
        self.group = group
    ...
    def __repr__(self):
        return "<{} {} {} {}>".format(self.group, self.type_key, self.size,
                                      self.filename)
        ...
        for filename in images:
            ...
            if isinstance(entry, dict):
                ...
                if bake_types.get(type_key) is not None:
                    ...
                    fallback = manifest_group or parse_filename(filename, tokens)[2]
                    found.append(FoundTexture(group or fallback, type_key, size, path,
                                              "manifest"))
                    continue
            if by_file:
            ...
            type_key, size, prefix = parse_filename(filename, tokens)
            if type_key is None:
                unmatched.append((os.path.join(group, filename) if group else filename,
                                  "cannot tell which map this is from the name"))
                continue
            name = group or prefix
            found.append(FoundTexture(name, type_key, size, path, "filename"))
