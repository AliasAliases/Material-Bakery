...
class BakeJob:
    ...
    def _dispatch_status(self, task):
        text = "baking {}/{}: {} — {} @{}px".format(
            self.index, self.total, task.group_name, task.bake_type.label, task.size)
        eta = self.eta_text()
    ...
    def _apply_antialias(self, task):
        ...
        try:
            ...
            return
        if mode == 'UPSCALE':
            self.report.warn("{} was rendered at {}px and upscaled to {}px".format(
                task.image_name, task.size, target))
