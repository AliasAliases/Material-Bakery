...
def add_default_maps(settings):
    ...
    for key, size in (("shader.base_color", 2048), ("shader.roughness", 2048),
                      ("shader.normal", 2048)):
        add_map(settings, key, size, dedupe=True)
    settings.map_index = 0


def add_map(settings, type_key, size, dedupe=False):
    """加一项。

    dedupe=False（Add 按钮）—— 直接复制一行，允许出现 (类型,尺寸) 相同的两行，
                              用户自己再改。要合并请用 Dedupe。
    ...
    if dedupe:
        for item in settings.maps:
            if item.type_key == type_key and item.size == size:
                return item
...
def dedupe_maps(settings):
    """删掉 (类型,尺寸) 重复的行，保留第一条。返回删掉的数量。"""
    seen = set()
    duplicates = []
    for index, item in enumerate(settings.maps):
        key = (item.type_key, item.size)
        if key in seen:
...
class MBAKERY_OT_AddMap(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        item = add_map(settings,
                       ...
                       current.size if current else properties.DEFAULT_SIZE)
        if item is None:
...
def bake_summary(context, settings):
    ...
    size = max([item.size for item in maps], default=0)
    rows.append(("Targets: {}".format(targets), 'OBJECT_DATAMODE'))
    rows.append(("Maps: {} item(s), up to {}px".format(len(maps), size), 'TEXTURE'))
