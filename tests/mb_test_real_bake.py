...
# ------------------------------------------------- 非正方形：真机烘一张出来
# ⚠ 这条不是"UI 上多了两个框"的复读，而是**真的用 Cycles 烘一张非正方形**：
#   照片子必须真的是 512x256、必须有内容、文件名必须按新规则渲染。
#   只在面板上改两个数字、引擎里其实还是正方形的话，这一条会直接挂。
print("\n=== 非正方形贴图（512 x 256，真机 Cycles）===")
wide_settings = pl.BakeSettings(
    target_mode=ss.MODE_COLLECTIONS,
    maps=pl.map_requests_from_pairs([("standard.roughness", 512, 256)]),
    margin=4,
)
wide_plan = pl.build_plan(bpy.context, wide_settings)
check("非正方形计划里尺寸是两条边",
      all(t.size_x == 512 and t.size_y == 256 for t in wide_plan.tasks),
      [(t.size_x, t.size_y) for t in wide_plan.tasks])
check("非正方形渲染尺寸也是两条边",
      all(t.render_dims == (512, 256) for t in wide_plan.tasks),
      [t.render_dims for t in wide_plan.tasks])
check("文件名按'两边都写完整像素'渲染（512x256，不是 512）",
      all("512x256" in t.image_name for t in wide_plan.tasks),
      [t.image_name for t in wide_plan.tasks])

wide_job = jb.BakeJob(bpy.context, wide_plan, wide_settings,
                      backend=bk.CyclesBackend(txmod.SceneTransaction(bpy.context),
                                               margin=4, samples=8,
                                               save_directory=OUT, save_files=False))
wide_job.run_to_completion()
check("非正方形烘焙成功", wide_job.report.failed == 0 and wide_job.report.done > 0,
      wide_job.report.summary())
for task in wide_plan.tasks:
    image = task.image
    if image is None:
        check("非正方形任务有图像", False, task.image_name)
        continue
    print("  {} -> {}".format(task.image_name, tuple(image.size)))
    check("  {} 真的是 512x256".format(task.bake_type.label),
          tuple(image.size) == (512, 256), tuple(image.size))
    pixels = list(image.pixels[:])
    check("  {} 不是空图".format(task.bake_type.label),
          max(pixels) > 0.001, max(pixels))
