...
check("UDIM 时命名用 {size}x 结构（不带 px、不带空格）",
      wide_task.image_name.startswith("Body-BaseColor-64x."), wide_task.image_name)
check("UDIM 时每张瓦片一个任务（4 张瓦片 -> 4 个任务）",
      len(uplan.tasks) == 4, len(uplan.tasks))
check("任务名带瓦片号",
      sorted(t.image_name for t in uplan.tasks) ==
      ["Body-BaseColor-64x.{}".format(t) for t in (1001, 1002, 1011, 1021)],
      sorted(t.image_name for t in uplan.tasks))
check("每个任务记了自己的瓦片号",
...
print("\n=== 阶段 8d：抗锯齿 ===")
check("OFF 时按原尺寸烘",
      pl.render_size(512, pl.BakeSettings(antialias='OFF', aa_scale=2)) == 512)
check("DOWNSCALE 时按 size×N 烘",
      pl.render_size(512, pl.BakeSettings(antialias='DOWNSCALE', aa_scale=2)) == 1024)
check("DOWNSCALE×3",
      pl.render_size(512, pl.BakeSettings(antialias='DOWNSCALE', aa_scale=3)) == 1536)
check("UPSCALE 时仍按原尺寸烘",
      pl.render_size(512, pl.BakeSettings(antialias='UPSCALE', aa_scale=2)) == 512)
...
check("超采样任务记录了两个尺寸",
      aa_plan.tasks[0].size == 64 and aa_plan.tasks[0].render_size == 128,
      (aa_plan.tasks[0].size, aa_plan.tasks[0].render_size))
...
print("\n=== 阶段 8e：自适应边距 ===")
check("关闭时就是基础边距",
      pl.effective_margin(4096, pl.BakeSettings(margin=16, adaptive_margin=False)) == 16)
check("打开时 1K 与基础值取大",
      pl.effective_margin(1024, pl.BakeSettings(margin=16, adaptive_margin=True)) == 16,
      pl.effective_margin(1024, pl.BakeSettings(margin=16, adaptive_margin=True)))
check("打开时 4K 会放大边距",
      pl.effective_margin(4096, pl.BakeSettings(margin=16, adaptive_margin=True)) == 33,
      pl.effective_margin(4096, pl.BakeSettings(margin=16, adaptive_margin=True)))
check("边距有上限（不超过 64）",
      pl.effective_margin(16384, pl.BakeSettings(margin=16, adaptive_margin=True)) == 64,
      pl.effective_margin(16384, pl.BakeSettings(margin=16, adaptive_margin=True)))
check("基础边距很大时不会被自适应改小",
      pl.effective_margin(1024, pl.BakeSettings(margin=48, adaptive_margin=True)) == 48,
      pl.effective_margin(1024, pl.BakeSettings(margin=48, adaptive_margin=True)))

print("\n=== 阶段 7/8f：报告存取 + 对照 ===")
report = rp.RunReport("unit")
report.start()
report.add(rp.TaskResult(group="Body", type_key="shader.base_color",
                         type_label="Base Color", size=64,
...
report.add(rp.TaskResult(group="Body", type_key="shader.normal", type_label="Normal",
                         size=64, image_name="BodyNormal - 64px",
