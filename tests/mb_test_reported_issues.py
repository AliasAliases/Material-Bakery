...
print("  {} 张 @{}px 估算 {:.1f} MB".format(
    len(mem_plan.tasks), mem_plan.tasks[0].size_x, one))
check("估算为正数", one > 0, one)
per_image = one / max(1, len(mem_plan.tasks))
nominal = (mem_plan.tasks[0].size_x * mem_plan.tasks[0].size_y
* 4 * 4 / (1024.0 * 1024.0))
...
for index in range(96):
    task = pl.BakeTask(
        ...
        targets=[], sources=[], size_x=2048, size_y=2048,
        image_name="Big{}".format(index),
        render_size_x=2048, render_size_y=2048)
    big.append(task)
...
if empty_session.job is not None:
    ...
    empty_session.report.add(report_mod.TaskResult(
        ...
        size_x=64, size_y=64, status=report_mod.STATUS_FAILED, error="simulated"))
...
# ⚠ 2026-09 的规则（用户口述）：两边都是 1024 的倍数就取**大的那条**写 k；
#   任一边不是就写完整像素（两边都写）。正方形仍然只写一个 token。
check("正方形：1024 -> 1k，不是 1024 倍数就写完整像素",
      (nm.size_token(1024), nm.size_token(512)) == ("1k", "512"),
      (nm.size_token(1024), nm.size_token(512)))
check("非正方形：两边都是 1024 倍数时取大的那条",
      (nm.size_token(2048, 1024), nm.size_token(1024, 4096)) == ("2k", "4k"),
      (nm.size_token(2048, 1024), nm.size_token(1024, 4096)))
check("非正方形：任一边不是 1024 倍数就写完整像素",
      nm.size_token(1024, 512) == "1024x512", nm.size_token(1024, 512))
check("非正方形：两条边都写，比例信息不丢（2048x1024 是 2k，不是 2048x1024）",
      nm.size_token(2048, 2048) == "2k" and nm.size_token(2048, 1024) == "2k",
      (nm.size_token(2048, 2048), nm.size_token(2048, 1024)))
check("UI 标签对非正方形写出真实比例",
      nm.size_label(2048, 1024) == "2048 × 1024" and nm.size_label(2048, 2048) == "2k",
      (nm.size_label(2048, 1024), nm.size_label(2048, 2048)))
...
check("预设版本已经升到 4（分辨率变成宽高两条边）",
      pr.PRESET_VERSION == 4, pr.PRESET_VERSION)
# ⚠ 老预设（只有 size）读进来必须还是**正方形、还是原来那个分辨率** ——
#   迁移就是干这个的，光看版本号不算数。
_legacy = {"version": 3, "maps": [{"type": "shader.base_color", "size": 512,
                                   "enabled": True, "options": {}}]}
_migrated = pr.migrate(_legacy)[0]["maps"][0]
check("v3 预设迁移成宽高两条边且行为不变",
      (_migrated.get("size_x"), _migrated.get("size_y")) == (512, 512), _migrated)
