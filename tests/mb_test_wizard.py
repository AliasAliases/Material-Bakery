...
check("新增项复制了当前行的类型与尺寸",
      last.type_key == settings.maps[0].type_key
      and last.size_x == settings.maps[0].size_x
      and last.size_y == settings.maps[0].size_y,
      (last.type_key, last.size_x, last.size_y))
...
op(bpy.ops.mbakery.add_map)
settings.maps[len(settings.maps) - 1].size_x = 4096
settings.maps[len(settings.maps) - 1].size_y = 4096
check("手动改尺寸后就是两项不同的",
      len({(m.type_key, m.size_x, m.size_y) for m in settings.maps})
      == len(settings.maps))
