...
for filename, expected_key, expected_size in cases:
    key, size_x, size_y, prefix = im.parse_filename(filename)
    check("反解 {} -> {}".format(filename, expected_key),
          key == expected_key and size_x == expected_size and size_y == expected_size,
          (key, size_x, size_y, prefix))

key, size_x, size_y, prefix = im.parse_filename("Common Parts 1-BaseColor-2k.png")
